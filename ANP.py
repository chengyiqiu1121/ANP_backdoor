...
parser.add_argument('--checkpoint', type=str, required=False, help='The checkpoint to be pruned')
...
parser.add_argument('--mask-file', type=str, required=False, help='The text file containing the mask values')
...
parser.add_argument('--path', type=str, default='/home/chengyiqiu/code/INBA/results/cifar10/phase/anp/20250103124013')
...
from classifier_models.defense.ANP.anp_batchnorm import NoisyBatchNorm2d, NoisyBatchNorm1d
def main():
    ...
    net = resnet18(num_classes=num_class, norm_layer=NoisyBatchNorm2d).to(f'cuda:{config.device}')
    ld = torch.load(f'{target_folder}/results.pth', map_location=device)
    # net.load_state_dict(ld['model'])
    load_state_dict(net, orig_state_dict=ld['model'])
